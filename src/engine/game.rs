...
impl Game {
    ...
    fn can_move_down(&self, target_particle: &SandParticle) -> bool {
        ...
        for particle in &self.particles {
            if particle.x == target_particle.x && particle.y == target_particle.y + BLOCK_SIZE {
                return false;
            }
        }
    ...
    fn can_move_down_sideways(&self, target_particle: &SandParticle, dx: i32) -> bool {
        ...
        if target_particle.x + dx > screen_width || target_particle.x + dx < 0. {
            return false;
        }
        ...
        for particle in &self.particles {
            if particle.x == target_particle.x + dx && particle.y == target_particle.y + BLOCK_SIZE
            {
                return false;
            }
        }
